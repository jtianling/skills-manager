...
```bash
# 1. Инициализация ~/.skills-manager/
npx skillsmgr setup

# 2. Установка навыков из официального репозитория Anthropic
npx skillsmgr install anthropic

# 3. Развёртывание навыков в текущий проект
cd your-project
npx skillsmgr init

# 4. Просмотр развёрнутых навыков
...
- Ненативные инструменты настраиваются путём создания моста через символические ссылки при выполнении `init` или `add`.
...
|---------|-----------|----------|
| `skillsmgr setup` | - | Инициализация `~/.skills-manager/` и создание `custom/example-skill/` |
...
| `skillsmgr init` | - | Интерактивное развёртывание в текущий проект |
...
| `--group <name>` | Сгруппировать навыки в `custom/<name>/` |
...
| `--group <name>` | Пакетное развёртывание всех навыков из пользовательской группы |
...
`install`, `init`, `add` и `uninstall` используют интерактивный селектор со следующими клавишами:
...
- `custom/`: локальные навыки, сгруппированные навыки и навыки, явно установленные как пользовательские
