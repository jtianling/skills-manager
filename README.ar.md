...
```bash
# 1. تهيئة ~/.skills-manager/
npx skillsmgr setup

# 2. تثبيت المهارات من مستودع Anthropic الرسمي
npx skillsmgr install anthropic

# 3. نشر المهارات في المشروع الحالي
cd your-project
npx skillsmgr init

# 4. فحص المهارات المنشورة
...
- تُهيَّأ الأدوات غير الأصلية بإنشاء جسر روابط رمزية أثناء `init` أو `add`.
...
|---------|-------|-------------|
| `skillsmgr setup` | - | تهيئة `~/.skills-manager/` وإنشاء `custom/example-skill/` |
...
| `skillsmgr init` | - | نشر تفاعلي في المشروع الحالي |
...
| `--group <name>` | تجميع المهارات تحت `custom/<name>/` |
...
| `--group <name>` | نشر دفعي لجميع المهارات من مجموعة مخصّصة |
...
يستخدم كل من `install` و`init` و`add` و`uninstall` محدّدًا تفاعليًا بالاختصارات التالية:
...
- `custom/`: المهارات المحلية والمهارات المجمّعة والمهارات المثبّتة صراحة كمخصّصة
