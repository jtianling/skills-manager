...
3. 没有 skill 时尝试 commands, 都没有 → 返回 false
4. 有 skill 时:
...
1. 提取仓库名: 从 URL 末尾匹配 `/([^/]+?)(\.git)?$`
2. 确定目标目录:
   - `anthropic` → `official/anthropic`
   - `--custom` → `custom/{repoName}`
   - 默认 → `community/{repoName}`
3. 如果目标已存在 → `git pull` 更新
4. 不存在 → `git clone --depth 1` 浅克隆
5. 克隆后扫描 skills:
   - 对于 anthropic, 检查 `skills/` 子目录
   - 在根目录或 skills/ 下查找包含 SKILL.md 的目录
6. 同时统计 commands 数量
7. 无 skill 且无 command → `process.exit(1)`
8. 有 skill 且非 `--all` → 提示选择
