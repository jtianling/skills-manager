...
#### Scenario: 找到多个匹配
- **WHEN** 中央仓库中有多个名为 `code-review` 的 skill (不同 source)
- **THEN** 提示用户选择 source (复用现有的多匹配选择逻辑)
...
所有带参数的流程 (skill name / provider-repo / URL) 在 skill 选择后 SHALL 进入 agent 选择.

#### Scenario: 无 -a 无 -s 进入交互选择
- **WHEN** 用户未指定 `-a` 或 `-s` 标志
- **THEN** 显示 agent 选择 UI (复用现有的 tool 选择逻辑)

### Requirement: -a/--agent 标志指定 agent

`-a`/`--agent` 标志 SHALL 接受逗号分隔的 agent 列表, 跳过交互选择.
